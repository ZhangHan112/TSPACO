...
TSPACO::TSPACO(int Iteraiton,int AntNumber,double Evaporation,int Run):
            iter(Iteraiton),num(AntNumber),rate(Evaporation),run(Run){
...
void TSPACO::ACO(){
    ...
    for(int r=0;r<run;r++){
        ...
        cout << "第 " << r+1 << " run, " << "Best: " <<Best << endl;
        avg += Best;
    }
    avg /= run;
    cout << "Average: " << avg << endl;
    ofstream output("output1.dat");
    for(int i=0;i<iter;i++){
        res[i]/=run;
        output << i << " " << res[i] << endl;
    }
...
solution TSPACO::Travel(int start){
    ...
    while(header){
        ...
        if(random<0.5){ // decide whether to use roulette wheel
